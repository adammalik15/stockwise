...
export default function HalalBadge({ result, ticker }: Props) {
  ...
  const scoreColor =
    result.status === 'HALAL' ? '#00d4aa' :
    result.status === 'DOUBTFUL' ? '#ffd166' :
    result.status === 'NOT_HALAL' ? '#ff4d6d' :
    '#4d9fff';
  ...
  const criteria = [
    ...
    },
    {
      ...
      desc: 'Debt must be ≤ 30% of market capitalization',
      status: result.financial_check.debt_ratio_flag === null
        ? null
        : !result.financial_check.debt_ratio_flag,
    },
    {
      label: 'Non-Permissible Income',
      desc: 'Haram revenue must be ≤ 5% of total revenue',
      status: result.business_check.passed ? null : false,
    },
    {
      label: 'Interest-Bearing Assets',
      desc: 'Interest assets must be ≤ 30% of market cap',
      status: null,
    },
  ];
              ...
              {criteria.map((c, i) => (
                <div key={i} className="flex items-start gap-2.5">
                  <span className="text-sm shrink-0 mt-0.5">
                    {c.status === true ? '✅' : c.status === false ? '❌' : '⚠️'}
                  </span>
                  <div>
