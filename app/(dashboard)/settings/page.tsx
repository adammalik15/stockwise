...
import { Moon, Sun, Monitor, Save, Check, Palette, Bell, Eye } from 'lucide-react';
    ...
    <div className="space-y-6 page-enter max-w-xl">
      ...
      <div className="card">
        ...
        <p className="text-[10px] text-muted mt-3">
          Theme applies immediately when you save.
        </p>
      </div>
      ...
      <div className="card">
        ...
        <div className="space-y-4">
          <Toggle
            ...
            desc="Show AAOIFI-based Halal analysis on stock pages"
            value={settings.show_halal_badge}
            onChange={v => update('show_halal_badge', v)}
          />
          <Toggle
            label="News & Sentiment"
            desc="Show news panel on stock detail pages"
            value={settings.show_news}
            onChange={v => update('show_news', v)}
          />
          <Toggle
            label="AI Recommendations"
            desc="Show AI-powered buy/hold/sell signals"
            value={settings.show_ai_recommendations}
            onChange={v => update('show_ai_recommendations', v)}
          />
          <Toggle
            label="Insider Transactions"
            desc="Show SEC insider buying/selling activity"
            value={settings.show_shareholders}
            onChange={v => update('show_shareholders', v)}
          />
          <Toggle
            label="Compact View"
            desc="Smaller cards with less whitespace"
      ...
      <div className="card">
        ...
        <div className="space-y-1.5 text-xs text-secondary">
          ...
          <p>Halal screening — AAOIFI standards</p>
          <p className="text-muted pt-2 leading-relaxed">
            For informational purposes only. Not financial advice.
            Always verify Halal compliance on Musaffa.com.
          </p>
        </div>
      </div>
    ...
    <div className="flex items-center justify-between gap-4">
      ...
      <button
        ...
        className={`relative w-12 h-6 rounded-full transition-colors duration-200 shrink-0 ${
          value ? 'bg-accent-green' : 'bg-surface-4'
        }`}
      >
